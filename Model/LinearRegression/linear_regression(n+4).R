...
# 데이터 불러오기
ro_data_mice4 <- read.csv("C:/Users/eourm/Desktop/융합팀플/선형회귀/RobustScaler/mice_df_robust4.csv")
str(ro_data_mice4)
...
pca_cov <- prcomp(ro_data_mice4[,-c(1,28)])#공분산 행렬 이용
...
pca <- prcomp(ro_data_mice4[,-c(1,28)], scale = T) #상관계수 행렬 이용
plot(pca, type = "l")
...
plot(test4[2:32,2], type = 'o', col = 'red')#실제 최고기온
...
shapiro.test(model_fwd3_d4$residuals[1:4999])#정규성 가정??독립성 가정이 깨지는거 아니냐??
car::durbinWatsonTest(model_fwd3_d4)#독립성 가정
car::ncvTest(model_fwd3_d4)#등분산성 검정 ??이것도 엉망인거 같은데??
